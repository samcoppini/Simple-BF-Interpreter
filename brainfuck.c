...
void add_loop_end(Commands *commands, int loop_start) {
	//If the previous command is a loop ending, we don't need an extra one,
	//so just make sure to adjust the loop's beginning
	if (commands->cmds[commands->num_commands - 1].type == CMD_LOOP_END) {
		commands->cmds[loop_start].change_val = commands->num_commands - 1;
		return;
	}
